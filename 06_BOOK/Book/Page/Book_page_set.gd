...
@onready var page_viewport_render = $SubViewport
...
func _ready():
	#init_timer.start()
	set_content.connect(_on_set_content)
...
func show_this_page(page_index : int):
	...
	var instance_this_page = _global_datas.all_book_page_data[page_index].pages.instantiate()
